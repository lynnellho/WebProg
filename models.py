...
# Create your models here.class AccountUser(models.Model):
    account_user_id = models.UUIDField(
        primary_key=True, default=RandomUUID, editable=False, unique=True)
    account_user_related_user = models.CharField(max_length=255, null=False)
    ...
    def friendly_profile(self):
        return mark_safe(u"%s <%s>") % (
            escape(self.account_user_id), escape(self.account_user_student_number),
            escape(self.account_user_name), escape(self.account_user_created_by),
            escape(self.account_user_updated_by), escape(self.account_user_created_date),
